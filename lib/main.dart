  ...
  void startFluidNavigation() {
    ...
    _navigationTicker = createTicker((Duration elapsed) {
      ...
      if (predictedPos != null && kinematicFilter.lastRealPos != null) {
        ...
        // ═══ CORRIGÉ : la flèche pointe en priorité du point JAUNE (GPS
        // brut) vers le point BLEU (cible), au lieu d'un cap calculé entre
        // deux positions d'animation successives de la flèche elle-même.
        // Cette dernière méthode pouvait donner un cap "vers rien" quand la
        // flèche restait quasi immobile en attendant le GPS (distMoved trop
        // faible) : on avait un cap qui ne représentait plus la vraie
        // direction de progression sur l'itinéraire.
        bool bearingFromGpsToTarget = false;
        ...
        if (yellowPos != null && bluePos != null) {
          ...
          if (yellowToBlueDist > 1.5) {
            ...
            bearingFromGpsToTarget = true;
          }
        }

        // 🔋 REPLI : CAP PAR LA TRAJECTOIRE DE LA FLÈCHE (utilisé seulement
        // si jaune/bleu sont trop proches pour donner un cap fiable, par ex.
        // entre deux fixes GPS pendant que la flèche avance seule).
        if (!bearingFromGpsToTarget && _currentAnimatedPos != null) {
