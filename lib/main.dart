  ...
  void startFluidNavigation() {
    ...
    _navigationTicker = createTicker((Duration elapsed) {
      ...
      if (predictedPos != null && kinematicFilter.lastRealPos != null) {
        // 🔧 CAP STABLE : par défaut on GARDE le dernier cap connu au lieu
        // de retomber sur le cap GPS brut (bruit pur à pied, ce qui faisait
        // tourner la flèche sans raison en marchant).
        double targetBearing = _lastBearing;

        // 🔋 CAP PRINCIPAL : TRAJECTOIRE RÉELLE DE LA FLÈCHE, mesurée sur un
        // historique d'environ 0.9s (au lieu du déplacement d'UNE seule
        // frame). L'ancien seuil de 5cm/frame ne pouvait quasiment jamais
        // être atteint à 3-4 km/h (~1.5cm/frame à 60fps), donc ce cap
        // n'était presque jamais utilisé en mode marche.
        bool bearingFromPath = _lastBearing != 0.0;

        if (_arrowTrail.length >= 2) {
          final LatLng oldP = _arrowTrail.first.pos;
          final double disp = Geolocator.distanceBetween(oldP.latitude,
              oldP.longitude, predictedPos.latitude, predictedPos.longitude);

          // Sur ~0.9s, même à 3km/h on parcourt ~0.8m : un seuil de 1m
          // reste atteignable, contrairement à l'ancien seuil par frame.
          if (disp >= 1.0) {
            double pathBearing = Geolocator.bearingBetween(oldP.latitude,
                oldP.longitude, predictedPos.latitude, predictedPos.longitude);
        ...
        // 🎯 REPLI : CAP JAUNE (GPS brut) → BLEU (cible). Utilisé UNIQUEMENT
        // au démarrage, quand aucun cap fiable n'est encore connu (ni cap
        // précédent, ni trajectoire assez longue). Dès qu'un cap existe, on
        // ne repasse plus jamais par le cap GPS brut.
        if (!bearingFromPath) {
          ...
          if (yellowPos != null && bluePos != null) {
            ...
            // On ignore ce cap si jaune et bleu sont quasi confondus (bruit
            // GPS / à l'arrêt) : un cap calculé sur <1.5m n'est pas fiable.
        ...
        _pushArrowTrail(snappedPos); // 🔧 alimente l'historique pour le cap stable
...
// ── AJOUT : MISE À JOUR DES STATS ET DISTRIBUTION DES BADGES ──
  ...
  void navigateToDestination({bool validateWalkingLegs = false}) async {
    ...
    final bool isWalkMode = homeController.currentTravelMode.value == TravelMode.walking ||
  ...
  void _checkRouteLogic(LatLng raw, LatLng snapped) {
    ...
    final double maxAccuracy = kf.isWalking ? kf.maxAccuracyWalking : kf.maxAccuracyVehicle;
  ...
  void _onAccel(UserAccelerometerEvent e) {
    ...
    if (instDeceleration > brakeThresh) {
      ...
      if (_isBraking) {
        ...
        _brakeCooldownUntil = now.add(const Duration(milliseconds: _cooldownMs));
      }
      _isBraking = false;
      _brakeCandidateStart = null;
      _deceleration = 0.0;
    ...
    if (instAcceleration > accelThresh) {
      ...
      if (_isAccelerating) {
        _accelCooldownUntil = now.add(const Duration(milliseconds: _cooldownMs));
      }
      _isAccelerating = false;
      _accelCandidateStart = null;
      _acceleration = 0.0;
...
class KinematicFilter {
  ...
  void updateRealPosition(Position rawPos, {bool isWalkingMode = false}) {
    ...
    final bool likelyAccelerating =
        accelDetector.isAccelerating || rawInstantDecel < -(_hardBrakeDecel * 0.5);
    ...
    if (_vehicleSpeedMps <= 0.01) {
      ...
      _vehicleSpeedMps =
          (_vehicleSpeedMps * (1.0 - speedSmoothing)) + (vInst * speedSmoothing);
    }
  ...
  void _applyBrakingFusion(double dt) {
    final double fusedBrake = math.max(_gpsBrakeIntensity, _accelBrakeIntensity);
  ...
  double _kinematicLookaheadDistance(double v0, double t) {
    ...
    return d.isNaN || d.isInfinite ? (v0 * t).clamp(0.0, double.infinity) : math.max(0.0, d);
  }
  ...
  LatLng? predictNextPosition(List<LatLng> routePolyline) {
    ...
    if (_isNewGpsPoint) {
      ...
      // ═══ AJOUT : VÉRIFICATION DE COHÉRENCE BLEU ↔ JAUNE ═══
      // Filet de sécurité supplémentaire : le point bleu (cible) ne doit
      // jamais se retrouver à une distance du point jaune (GPS brut) qui
      // ne serait pas justifiée par la vitesse réellement mesurée. Si ça
      // arrive (ex: _vehicleSpeedMps momentanément faussé), on recale la
      // cible à la distance maximale physiquement plausible.
      final double maxCoherentGap =
          (_vehicleSpeedMps * _normalLookaheadSeconds) +
              (_isWalking ? 4.0 : 8.0); // marge de sécurité (bruit GPS)

      final double blueYellowGap = Geolocator.distanceBetween(
        _targetPos!.latitude,
        _targetPos!.longitude,
        _snappedGpsPos!.latitude,
        _snappedGpsPos!.longitude,
      );

      if (blueYellowGap > maxCoherentGap) {
        _targetPos = _advanceForwardAlongRoute(
          _snappedGpsPos!,
          maxCoherentGap,
          routePolyline,
      ...
      // ── Détection d'arrêt confirmé ──
      // ═══ AJOUT : en marche, l'accéléromètre confirme un vrai arrêt plus
      // vite que le GPS seul (la vitesse GPS étant très bruitée à ces
      // faibles allures) → moins de "rampement" de la flèche à l'arrêt.
