  ...
  void startFluidNavigation() {
    ...
    _navigationTicker = createTicker((Duration elapsed) {
      ...
      if (predictedPos != null && kinematicFilter.lastRealPos != null) {
        ...
        // 🔋 CAP PRINCIPAL : TRAJECTOIRE DE LA FLÈCHE (recalculé à CHAQUE
        // frame à 60 FPS → c'est ce qui donne un mouvement fluide, sans
        // saccade). Le cap "jaune→bleu" ne se met à jour qu'1x/sec (au
        // rythme du GPS) : le garder en priorité gèle le cap pendant que la
        // position continue d'avancer chaque frame, ce qui désynchronise
        // position et orientation et donne l'effet de tremblement.
        bool bearingFromPath = false;
        ...
        // quand la flèche ne bouge pas assez (distMoved <= 5cm, ex: elle
        // attend le GPS) pour donner un cap fiable via la trajectoire — au
        // lieu de laisser un cap figé "vers rien" comme avant. Comme ce cas
        // correspond à une flèche quasi immobile, l'absence de mise à jour
        // 60fps ici ne crée pas de désync visible (rien ne bouge de toute
        // façon).
        if (!bearingFromPath) {
  ...
  LatLng? predictNextPosition(List<LatLng> routePolyline) {
    ...
    if (_isNewGpsPoint) {
      ...
      if (acceptFix &&
        ...
        final double jump = _signedRouteDistance(
        ...
        final double plausibleMax = (maxPlausibleSpeed * elapsed) + 10.0;
        ...
        if (jump > plausibleMax && _lastRawAccuracy > degradedAccuracy) {
          acceptFix = false;
        }
      }
