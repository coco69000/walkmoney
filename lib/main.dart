...
class AccelerometerBrakeDetector {
  ...
  static const double _minBrakeMs = 60;
  ...
  void _onAccel(AccelerometerEvent e) {
    // Magnitude horizontale (x,y) — on ignore z (gravité)
    ...
    if (_buffer.length >= 2) {
      ...
      // delta > 0 → on accélère
      _deceleration = delta < 0 ? -delta : 0.0;
      _acceleration = delta > 0 ? delta : 0.0;
    }
    ...
    if (_deceleration > brakeThresh) {
      if (!_isBraking) {
        _isBraking = true;
        _brakeStart = now;
      }
      // Valider seulement si durée > seuil
      if (_brakeStart != null &&
          now.difference(_brakeStart!).inMilliseconds >= _minBrakeMs) {
        // Freinage confirmé
      }
    } else {
      if (_isBraking) {
...
class KinematicFilter {
  ...
  double _accelBrakeIntensity = 0.0;
  ...
  void updateRealPosition(Position rawPos, {bool isWalkingMode = false}) {
    ...
    // ═══════════════════════════════════════════════════
    // 🛑 DÉTECTION FREINAGE : FUSION GPS + ACCÉLÉROMÈTRE
    // ═══════════════════════════════════════════════════

    // 1. Détection GPS (existante, 1 Hz)
    double gpsDeceleration = 0.0;
    if (dt > 0.1) {
      gpsDeceleration = (_previousVehicleSpeedMps - _vehicleSpeedMps) / dt;
    }
    double gpsBrakeIntensity = 0.0;
    if (gpsDeceleration > _hardBrakeDecel) {
      gpsBrakeIntensity = _clamp(
    ...
    // 2. Détection Accéléromètre (60 Hz, instantanée)
    _accelBrakeActive = accelDetector.isBraking;
    if (_accelBrakeActive) {
      // Convertir la décélération accéléromètre en intensité 0→1
    ...
    // 3. FUSION : prendre le MAX des deux sources
    //    L'accéléromètre réagit en ~50ms, le GPS en ~1000ms
    final double fusedBrake = math.max(gpsBrakeIntensity, _accelBrakeIntensity);

    if (fusedBrake > 0.0) {
      _brakingIntensity = math.max(_brakingIntensity, fusedBrake);
    } else {
      // Décroissance douce quand aucun freinage détecté
